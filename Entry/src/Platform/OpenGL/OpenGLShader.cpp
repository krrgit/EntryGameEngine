...
namespace Entry {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			{
				ET_CORE_ERROR("Could not read from file '{0}'", filepath);
			}
		}
		else
		{
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			ET_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		ET_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now");
		...
		if (isLinked == GL_FALSE)
		{
			...
			ET_CORE_ERROR("{0}", infoLog.data());
			ET_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
